...
if [ "$DOCKER_ARCH" = "multiarch" ]; then
	...
	if [ $LOCAL -eq 1 ]; then
		...
		DOCKER_EXPORT="($DOCKER_ARCH/$DOCKER_IMAGE ghcr.io/Flusinerd/vehicle-services/$DOCKER_IMAGE:prerelease)"
	else
...
if [ $? -eq 0 ]; then
	echo "docker image tag $DOCKER_ARCH/$DOCKER_IMAGE ghcr.io/Flusinerd/vehicle-services/$DOCKER_IMAGE:prerelease"
	docker image tag $DOCKER_ARCH/$DOCKER_IMAGE ghcr.io/Flusinerd/vehicle-services/$DOCKER_IMAGE:prerelease
